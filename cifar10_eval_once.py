...
tf.app.flags.DEFINE_string('eval_dir', '/scratch2/dungn/cifar10_eval',
                           """To write eval logs""")
tf.app.flags.DEFINE_string('eval_data', 'test', """test or train""")
...
def eval_once(saver, summary_writer, top_k_op, summary_op, checkpoint_path, dropout):
    """Run Eval once.
...
def evaluate():
    ...
    with tf.Graph().as_default() as graph:
        ...
        eval_input = cifar10.inputs(eval_data=eval_data)

        iterator = eval_input.make_one_shot_iterator()
        ...
        for checkpoint_path in tf.train.get_checkpoint_state(FLAGS.checkpoint_dir).all_model_checkpoint_paths:
            eval_once(saver, summary_writer, top_k_op, summary_op, checkpoint_path, dropout)
